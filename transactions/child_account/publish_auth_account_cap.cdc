...
transaction(parentAddress: Address) {
    ...
    prepare(signer: AuthAccount) {
        ...
        if !signer.getCapability<&AuthAccount>(ChildAccount.AuthAccountCapabilityPath).check() {
            self.authAccountCap = signer.linkAccount(/private/AuthAccountCapability)!
        } else {
            self.authAccountCap = signer.getCapability<&AuthAccount>(/private/AuthAccountCapability)
        }
