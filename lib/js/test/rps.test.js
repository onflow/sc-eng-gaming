...
describe("rockpaperscissorsgame", ()=>{
  ...
  beforeEach(async () => {
    ...
    await mintFlow(serviceAccount, 10000.0);
    ...
    await deployContract({ to: serviceAccount, name: "DynamicNFT" });
    await deployContract({ to: serviceAccount, name: "GamePieceNFT" });
    await deployContract({ to: serviceAccount, name: "TicketToken" });
    await deployContract({ to: serviceAccount, name: "ArcadePrize" });
    await mintFlow(gameAdmin, 10000.0);
