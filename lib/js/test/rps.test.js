...
describe("rockpaperscissorsgame", ()=>{
  ...
  test("player should be able to submit Move to a single player Match", async () => {
    ...
    await submitMovePasses(playerOne, parseInt(matchIDs[0]), 0);
    // Sixth step: try to submit another move & fail
    await submitMoveReverts(playerOne, parseInt(matchIDs[0]), 0);
  ...
  test("player should be able to submit autoplayer Move to a single player Match", async () => {
    ...
    await submitBothSingleplayerMovesPasses(playerOne, parseInt(matchIDs[0]), 0); // Any decent RPS player will always play rock on the first round
  });
  ...
  test("player should be able to complete SinglePlayer match", async () => {
    ...
    await shallPass(
      sendTransaction({
        name: "rock_paper_scissors_game/game_player/submit_move",
    ...
    // TODO: Check NFT returned is the NFT that was escrowed
    // TODO: Check the win/loss record on the NFT
    // TODO: Check the assigned moves on the NFT
  });
  ...
  test("Two players should be able to complete a Match", async () => {
    ...
    await submitMovePasses(playerOne, parseInt(playerOneInPlayMatchIDs[0]), 0);
    ...
    await submitMovePasses(playerTwo, parseInt(playerTwoInPlayMatchIDs[0]), 0);
  ...
  // TODO: Player can play match they were added to
  // TODO: Try to game contract
  // - submit first move, then submit second with post condition
  // - submit both moves, then post-condition on match result;
  // TODO: Remove attachments
  // TODO: Test contract methods
  // - Get total win loss
  // - Get win loss
});