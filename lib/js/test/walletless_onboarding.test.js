...
const get_child_address_from_creator = fs.readFileSync(path.resolve(__dirname, "./../../../scripts/account_creator/get_child_address_from_public_key_on_creator.cdc"), {encoding:'utf8', flag:'r'});
...
describe("Walletless onboarding", ()=>{
  ...
  beforeEach(async () => {
    ...
    await deployContract({ to: gameAccount, name: "utility/AccountCreator" });
    await deployContract({ to: gameAccount, name: "DynamicNFT" });
    await deployContract({ to: gameAccount, name: "GamePieceNFT" });
  ...
  test("Game should be able to create and setup a child account", async () => {
    ...
    await shallPass(
      sendTransaction({
        name: "linked_accounts/setup_account_creator",
        args: [],
        signers: [gameAccount]
      })
    );
    ...
    await shallPass(
      sendTransaction({
        name: "onboarding/walletless_onboarding",
        args: [
          pubKey, 
          "10.0",
          "1",
  ...
  test("Game should be able to associate child to parent", async () => { // parent should accept child!
    ...
    await shallPass(
      sendTransaction({
        name: "linked_accounts/add_as_child_multisig",
        args: [
          "ACCOUNT_NAME",
          "ACCOUNT_DESCRIPTION",
          "THUMBNAIL_URL",
